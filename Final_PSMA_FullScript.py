...
for current in subjs_name:
    ...
    for file in glob.glob(subj_dir +'/' +'*.xlsx'):
        shutil.move(os.path.join(subj_dir, file), info)
    for file in glob.glob(subj_dir +'/' +'*ROI*'):
        shutil.move(os.path.join(subj_dir, file), ROIs)
    for file in glob.glob(subj_dir +'/' +'CEtum*'):
        shutil.move(os.path.join(subj_dir, file), ROIs)
    for file in glob.glob(subj_dir +'/' +'crescent*'):
        shutil.move(os.path.join(subj_dir, file), ROIs)    
    ...
    if os.path.exists(ROIs +'/CEtum.nii'):
        CEtum = sitk.ReadImage(ROIs +'/CEtum.nii') #read CEtum roi
    FET_roi = sitk.ReadImage(ROIs +'/FET_ROI.nii') #read FET roi
    TER_roi = sitk.ReadImage(ROIs +'/PSMA_ROI.nii') #read TER roi
    crescentFET = sitk.ReadImage(ROIs +'/crescentFET.nii') #read crescent FET roi drawn on FET CT
    crescentTER = sitk.ReadImage(ROIs +'/crescentTER.nii') #read crescent TER roi drawn on TER CT
    ...
    #Resample FET ROI into CT FET space
    FETroi_inFETCT = Resample_roi(FET_roi, CT_FET)
    sitk.WriteImage(FETroi_inFETCT, ROIs +'/'+ subj_name +'_FETroi_inFETCT.nii')
    ...
    #Resample TER ROI into CT TER space
    TERroi_inTERCT = Resample_roi(TER_roi, CT_TER)
    sitk.WriteImage(TERroi_inTERCT, ROIs +'/'+ subj_name +'_TERroi_inTERCT.nii')
    
    ##Registration of T1CE, PET TER and CT TER to CT FET
    fixed_image = CT_FET
    if T1CE != 0:
        moving_images = [T1CE, CT_TER, PET_TER]
    else:
        moving_images = [CT_TER, PET_TER]
        
    for moving_image in moving_images:
        
        if subj_name == 'DA003' or 'GF002':
            
            print('Registration optimization method: Multires(4,2,1)')
            #Register and resample image
            if moving_image is CT_TER:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'same', 'Multires(4,2,1)')
            elif moving_image is T1CE:
        ...
        elif subj_name == 'AB001':
            
            print('Registration optimization method: Multires(8,4,2,1)')
            #Register and resample image
            if moving_image is CT_TER:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'same', 'Multires(8,4,2,1)')
            elif moving_image is T1CE:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'different', 'Multires(8,4,2,1)')
            elif moving_image is PET_TER:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'different', 'ExplorExploit')
                
        elif subj_name == 'FF004':
            
            print('Registration optimization method: ExplorExploit')
            #Register and resample image
            if moving_image is CT_TER:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'same', 'Multires(8,4,2,1)')
            elif moving_image is T1CE:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'different', 'MetricEvaluate')
            elif moving_image is PET_TER:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'different', 'Multires(8,4,2,1)')
            ...
            #Register and resample image
            if moving_image is CT_TER:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'same', 'ExplorExploit')
            elif moving_image is T1CE:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'different', 'ExplorExploit')
            elif moving_image is PET_TER:
                moving_resampled, final_transform = RegisterResample_image(fixed_image, moving_image, 'different', 'ExplorExploit')
        ...
        if moving_image is T1CE:
            sitk.WriteImage(moving_resampled, reg_imgs +'/'+ subj_name +'_T1CE_inFETCT.nii')
            sitk.WriteTransform(final_transform, reg_mtrx +'/'+ subj_name +'_T1CE_2_FETCT.tfm')
        elif moving_image is CT_TER:
    ...
    #Apply transform to ROIs and resample them to FET CT image
    print('Start registration for '+current+ ' rois')
    
    if os.path.exists(ROIs +'/CEtum.nii'):
        CEtum_inFETCT = apply_tfm_to_roi(CEtum, CT_FET, T1CE2FETCT_tfm)
        sitk.WriteImage(CEtum_inFETCT, ROIs +'/'+ subj_name +'_CEtum_inFETCT.nii')
    
    TERroi_inFETCT = apply_tfm_to_roi(TER_roi, CT_FET, PETTER2FETCT_tfm)
    sitk.WriteImage(TERroi_inFETCT, ROIs +'/'+ subj_name +'_TERroi_inFETCT.nii')
    
    ##Generate additional rois
    print('Generate additional  rois for '+current)
    
    #Generate mirror image CTRL rois 
    FETroi_inFETCT_CTRL = flip(FETroi_inFETCT)
    sitk.WriteImage(FETroi_inFETCT_CTRL, ROIs +'/'+ subj_name +'_FETroi_inFETCT_CTRL.nii')
    
    TERroi_inFETCT_CTRL = flip(TERroi_inFETCT)
    sitk.WriteImage(TERroi_inFETCT_CTRL, ROIs +'/'+ subj_name +'_TERroi_inFETCT_CTRL.nii')
    
    TERroi_inTERCT_CTRL = flip(TERroi_inTERCT)
    sitk.WriteImage(TERroi_inTERCT_CTRL, ROIs +'/'+ subj_name +'_TERroi_inTERCT_CTRL.nii')
    
    #Generate crescent-shaped CTRL rois       
    crescentFET = generate_3Dcrescent_roi(crescentFET)
    sitk.WriteImage(crescentFET, ROIs +'/'+ subj_name +'_crescentFET.nii')
    crescentTER = generate_3Dcrescent_roi(crescentTER)
    sitk.WriteImage(crescentTER, ROIs +'/'+ subj_name +'_crescentTER.nii')
    
    #Generate FET thresholded values sub-rois of CEtum roi    
    if os.path.exists(ROIs +'/'+ subj_name +'_CEtum_inFETCT.nii'):
        
        CEtumFETmask = generate_mask(PETFET_inFETCT,CEtum_inFETCT) #Generate a mask of the FET positive CE tumour roi
        sitk.WriteImage(CEtumFETmask, ROIs +'/'+ subj_name +'_CEtumFETmask.nii')
        
        FETminCEtum = generate_thresholded_roi(CEtumFETmask, low_thr=0.000001, high_thr=2.2) #Generate a binary mask of the voxel with FET intensity <= 2.2 in the CEtum roi
        sitk.WriteImage(FETminCEtum, ROIs +'/'+ subj_name +'_FETminCEtum.nii')  
        
        FETmaxCEtum = sitk.Cast(CEtum_inFETCT-FETminCEtum, sitk.sitkUInt16)
        sitk.WriteImage(FETmaxCEtum, ROIs +'/'+ subj_name +'_FETmaxCEtum.nii')
        
    # Generate subfolders into the ROIs folder    
    os.mkdir(ROIs +'/Original ROIs')
    os.mkdir(ROIs +'/Registered ROIs')
    os.mkdir(ROIs +'/Mirror image CTRL ROIs')
    os.mkdir(ROIs +'/Crescent CTRL ROIs')
    os.mkdir(ROIs +'/subCEtum ROIs')
    ...
    # Move files into subfolders   
    for file in glob.glob(ROIs +'/' + subj_name +'_crescent*'):
        shutil.move(os.path.join(ROIs, file), CCTRL_ROIs)
    for file in glob.glob(ROIs +'/' + subj_name +'_FETmax*'):
        shutil.move(os.path.join(ROIs, file), subCE_ROIs)
    for file in glob.glob(ROIs +'/' + subj_name +'_FETmin*'):
        shutil.move(os.path.join(ROIs, file), subCE_ROIs)
    for file in glob.glob(ROIs +'/' + subj_name +'*mask*'):
        shutil.move(os.path.join(ROIs, file), subCE_ROIs)
    for file in glob.glob(ROIs +'/' + subj_name +'*_CTRL*'):
        shutil.move(os.path.join(ROIs, file), MCTRL_ROIs)
    for file in glob.glob(ROIs +'/' + subj_name +'*_in*'):
        shutil.move(os.path.join(ROIs, file), reg_ROIs)
    for file in glob.glob(ROIs +'/' + '*.nii'):
        shutil.move(os.path.join(ROIs, file), orig_ROIs)
